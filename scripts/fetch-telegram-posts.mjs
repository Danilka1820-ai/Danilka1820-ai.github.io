...
async function optimizeVideo(file, kind) {
  ...
  const ok = tool(FFMPEG, [
    ...
    // Потолок — 480p (854 точки по ширине). Сначала был Full HD, потом 720p;
    // спустились ещё на ступень, когда девять роликов заняли 24 МБ из 36 МБ
    // всего репозитория — две трети веса на девять файлов. На экране дневника,
    // где кадр идёт колонкой в 34rem, разница с 720p почти не видна, а весит
    // 480p вдвое меньше.
    //
    // Ниже опускать нельзя: 640 по ширине — это уже 360p, и оно совпало бы с
    // лёгкой версией. Тогда makeSmaller отказался бы её делать (правило «не
    // легче на 15% — не нужна»), второй уровень исчез бы совсем, и на канале
    // 400 кбит/с ролики перестали бы открываться вовсе.
    '-vf', `scale='min(${round ? 540 : 854},iw)':-2`,
    '-c:v', 'libx264', '-crf', round ? '32' : '30', '-preset', 'veryfast',
...
/* Средний уровень имел смысл, пока основной был Full HD: телефону иначе
   ...
   потолком 480p он тем более шире основного, и makeSmaller сам от него
