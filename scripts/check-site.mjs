...
шаг('у видео остаётся два уровня качества', () => {
  ...
  if (потолок <= 704){
    throw new Error(
      `потолок основной версии ${потолок} — лёгкая версия (640) больше не будет создаваться, ` +
      'и на слабом канале видео перестанет открываться; см. README, «Два качества»');
  }
});
